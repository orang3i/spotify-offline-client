...
// Function to download a song in its original format
async function downloadSong(song, outputDir) {
    ...
    const fileName = `${song.name.replace(/[^a-zA-Z0-9]/g, '_')}.webm`;
    ...
    try {
        ...
        for (const playlist of playlistsData) {
            ...
            for (const track of playlist.tracks) {
                await downloadSong(track, outputDir);
            }
        }
